...
def upload_and_poll(baseurl):
  print("Starting upload_and_poll function")

  try:
    print("Enter PDF filename>")
    local_filename = input().strip()
    print("Enter user id>")
    userid = input().strip()
    ...
    upload_res = requests.post(upload_url, json=payload)
    
    if upload_res.status_code != 200:
      print("Upload failed with status code:", upload_res.status_code)
    ...
    download_url = f"{baseurl}/results/{jobid}"
    while True:
      ...
      if status_code == 200:
        ...
        results = base64.b64decode(body).decode("utf-8")
        ...
        print("Server/network error. Status code:", status_code)
        print("Retrying...")
